...
const SadaCart = {
  ...
  initEventListeners() {
    ...
    document.addEventListener("click", (e) => {
      if (e.target.classList.contains("buy-now-btn") || e.target.closest(".buy-now-btn")) {
        ...
        // Get quantity from input field if on product detail page
  ...
  initMobileNavigation() {
    ...
    // Close menu when clicking on navigation links (except toggles)
    const navLinks = document.querySelectorAll('.mobile-nav-links a:not(.mobile-nav-toggle)')
  ...
  openMobileMenu() {
    ...
    if (mobileNavMenu) {
      ...
      const firstFocusable = mobileNavMenu.querySelector('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')
      if (firstFocusable) {
  ...
  closeMobileMenu() {
    ...
    if (mobileNavMenu && mobileNavMenu.classList.contains('show')) {
      ...
      // Close all submenus
      const openSubmenus = document.querySelectorAll('.mobile-nav-submenu.show')
